[Skip to contents](https://spark.apache.org/docs/latest/api/R/authors.html#main)
[ ![](https://spark.apache.org/images/spark-logo-rev.svg)](https://spark.apache.org/) [SparkR](https://spark.apache.org/docs/latest/api/R/index.html) 4.1.2
  * [Reference](https://spark.apache.org/docs/latest/api/R/reference/index.html)
  * [Articles](https://spark.apache.org/docs/latest/api/R/authors.html)
[SparkR - Practical Guide](https://spark.apache.org/docs/latest/api/R/articles/sparkr-vignettes.html)

  *

![](https://spark.apache.org/docs/latest/api/R/authors.html)
# Authors and Citation
## Authors
  * **[The Apache Software Foundation](https://www.apache.org/)**. Author, maintainer, copyright holder.

## Citation
The Apache Software Foundation (2026). _SparkR: R Front End for 'Apache Spark'_. https://www.apache.org https://spark.apache.org.

```
@Manual{,
  title = {SparkR: R Front End for 'Apache Spark'},
  author = {{The Apache Software Foundation}},
  year = {2026},
  note = {https://www.apache.org https://spark.apache.org},
}
```

## On this page
Developed by [ The Apache Software Foundation](https://www.apache.org/).
Site built with [pkgdown](https://pkgdown.r-lib.org/) 2.0.1.
Using [preferably](https://preferably.amirmasoudabdol.name/?source=footer) template.
